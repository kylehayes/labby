...
class StatusBarService {
  static const MethodChannel _channel =
      MethodChannel('gitlab_pipeline_monitor/status_bar');
  ...
  static Future<void> initialize() async {
    ...
    try {
      ...
      // Fail silently for platforms that don't support status bar
    }
  }
  ...
  static Future<void> updateStatus({
    ...
    // Debug: Status update for $projectName #${pipeline.id} - Status: $newStatus
  ...
  static PipelineStatus _calculateRollupStatus(
      GitLabPipeline pipeline, List<GitLabJob> jobs) {
    ...
    if (pipeline.status == 'success' &&
        jobs.every((job) => job.isSuccess || job.isCanceled)) {
  ...
  static Future<void> _updateStatusBarIcon(
      PipelineStatus status, String projectName, int pipelineId) async {
    try {
      final iconName = _getIconForStatus(status);
      final tooltip = _getTooltipForStatus(status, projectName, pipelineId);

      // Debug: Setting icon=$iconName, tooltip=$tooltip
      ...
      // Failed to update status bar icon
    }
  }
  ...
  static String _getTooltipForStatus(
      PipelineStatus status, String projectName, int pipelineId) {
